...
$(() => {
  ...
  // list.forEach(l => parseMod(JSON.stringify(l)));
});
...
function parseMod(modString) {
  const mod = JSON.parse(modString);
  mods.push(mod);

  $('#mod-list').append(`
    <li>
      ${mod.slotName} - ${mod.numMods}
      <br />
      <div class="center">
        ${getCurrentSelection(mod)}
      </div>
    </li>
  `)
}

function getCurrentSelection(mod) {
  const id = mod.index;

  if (isNaN(modSelection[id])) {
    modSelection[id] = 0;
  }

  return `
    <button class="btn btn-secondary align-left" onclick="clickLeft(${id})">&#xab;</button>
    <span class="${mod.index}">${mod.modLabels[modSelection[id]].displayName}</span>
    <button class="btn btn-secondary align-right" onclick="clickRight(${id})">&#xbb;</button>
  `;
}

function clickLeft(id) {
  const mod = mods.find(m => m.index === id);

  const numMods = mod.numMods;
  const currentSelection = modSelection[id];

  if (currentSelection - 1 < 0) {
    modSelection[id] = numMods;
  } else {
    modSelection[id] = currentSelection - 1;
  }
...
function clickRight(id) {
  ...
  if (currentSelection + 1 > (numMods - 1)) {
    modSelection[id] = 0;
  } else {
    modSelection[id] = currentSelection + 1;
  }
...
function modifyVehicle(mod) {
  $(`.${mod.index}`).html(`${mod.modLabels[modSelection[mod.index]].displayName}`);

  alt.emit('updateLocalVehicle', {
    modType: mod.index,
    modIndex: mod.modLabels[modSelection[mod.index]].index,
  });
}
